...
namespace ht
{
		...
		const wchar_t* fileName)		// имя файла 
	{
		...
		LPVOID lp = MapViewOfFile(// подключение объекта файлового отображения к адресному пространству
			hm,
			FILE_MAP_ALL_ACCESS,
			0, 0, 0);
		if (!lp)
		...
		const Element* element)              // элемент
	{
		...
		WaitForSingleObject(htHandle->mutex, INFINITE);//ожидание сигнального состояния
